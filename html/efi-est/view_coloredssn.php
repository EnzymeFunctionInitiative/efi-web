...
if ($job_type === "NBCONN") {
    ...
    $cluster_has_results = $obj->get_has_results();
...
if ($job_type === "COLORSSN" || $job_type === "CLUSTER") {
    if (!isset($cluster_has_results)) {
        echo '<div class="update-message">The analysis was not able to be computed due to an unknown error.</div>';
    } else if ($cluster_has_results === false) {
        echo '<div class="update-message"><i class="fas fa-exclamation-triangle" style="color: red; font-size: 1.5em"></i><br>';
        echo 'The input SSN did not include UniProt IDs for the nodes.  The SSN likely was generated by the EFI-EST FASTA option from 1) FASTA files without UniProt IDs in the headers or 2) FASTA files with UniProt IDs in the headers but without using the "Read FASTA headers" option.';
        if ($job_type === "COLORSSN")
            echo '<br><br>Without UniProt IDs for the nodes, this tool cannot provide sets of IDs and sequences and SwissProt annotations per cluster.';
        else
            echo '<br><br>Without UniProt IDs for the nodes, this tool cannot provide sets of IDs and sequences, SwissProt annotations, MSAs, WebLogos, HMMs, consensus residues, and length histograms per cluster.';
        echo '</div>\n';
    }
}
