...
elseif (isset($_GET["stats-as-table"])) {
    ...
    for ($i = 0; $i < count($stats); $i++) {
        if ($i == 0) {
            ...
            if ($stats[$i]['Nodes'] == 0) {
                $rep_network_html .= "<td style='text-align:center;' colspan='3'>The output file was too large to be generated by EST.<!--  Please use a smaller %ID repnode or choose a different alignment score.--></td>\n";
            } else {
...
<h2 class="darkbg">Download Network Files</h2>
    ...
    <p>
        ...
        <a href='<?php echo $_SERVER['PHP_SELF'] . "?id=" . $_GET['id'] . "&key=" . $_GET['key'] . "&analysis_id=" . $_GET['analysis_id'] . "&as-table=1" ?>'><button class='mini'>Download</button></a>
    </p>
...
<?php if (!$full_network_html) { ?>
    <p><b>The output file was too large to be generated by EST.  Please use a repnode below or choose a different alignment score.</b></p>
<?php } else { ?>
    <table width="100%" border="1">
    <tr>
        <th></th>
